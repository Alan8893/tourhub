...
- add an authenticated Backend projection of the current user's active sessions;
- identify the current session server-side from the HttpOnly session cookie;
- expose only session ID, creation time, last-seen time, expiry time, and current-session marker;
- allow the user to revoke one owned non-current active session by session ID;
- keep session ownership, active-state checks, current-session protection, and revocation policy centralized in Backend services;
- record one safe semantic `account_session_revoked` AuditEvent in the same transaction as revocation;
- add a responsive session-management section to `/account` with explicit loading, empty, success, and failure states;
...
- unrelated, revoked, or expired session IDs are not disclosed and return 404;
...
- revoking an unrelated, revoked, expired, or unknown session returns 404 without revealing ownership;
...
- audit payload contains bounded session ID/state metadata only and excludes tokens, hashes, cookies, IP/device data, and authorization headers.
...
No migration is expected because `auth_sessions` already stores ownership, token hash, creation, last-seen, expiry, and revocation timestamps. Current Alembic head remains `h10023` unless implementation proves a persistence change is required.
