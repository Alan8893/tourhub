...
def get_purchase_checklist_service(session: Session = Depends(get_session)) -> PurchaseChecklistService:
    return PurchaseChecklistService(
        ...
        shopping_service=MealPlanShoppingService(shopping_list_service=ShoppingListService(session)),
    )
